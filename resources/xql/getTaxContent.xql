...
declare variable $path := 'xmldb:exist:///apps/theater-data/taxation/';
...
tr:nth-child(even) {
    ...
    <table
        ...
        cellpadding="10"
        cellspacing="0"
        style="font-size: 12px; width:100%; border-collapse: collapse; font-family: arial, sans-serif">{local:dispatch($node/node())}</table>
};
    ...
    then
        ...
        then
            ...
            <persName
                id='{$node/@key}'><a
                    style="color: inherit"
                    href="javascript:getPersonContentForTaxation('{$node/@key}', '{$node/text()}');">{$node}</a></persName>
            )
        else
            (
    ...
    then
        ...
        then
            ...
            <rs
                id='{$node/@key}'><a
                    style="color: inherit"
                    href="javascript:getWorkContentForTaxation('{$node/@key}', '{$node/text()}');">{$node}</a></rs>
            )
        else
            (
            <rs>{$node}</rs>
