Ext.define('TheaterTool.view.tabPanel.repertoire.work.WorkDetailsSection', {
    ...
    initComponent: function () {
        ...
        Ext.Ajax.request({
            ...
            success: function (result) {
                ...
                if (json.sprachen.length > 0 || json.instr.length > 0 || json.creation[0][0] !== '' || json.hoverview.length > 0 || json.gnd[0].length > 0 || json.wega[0].length > 0) {
                    ...
                    if (json.sprachen.length > 0) {
                        ...
                        for (var i = 1; i < json.sprachen.length; i++) {
                            spr += ', ' + json.sprachen[i];
                        }
                    ...
                    if (json.instr.length > 0) {
                        ...
                        for(var m = 0; m < json.instr.length; m++){
                            var spr = json.instr[m];
                            var onespr = spr[0];
                            for (var i = 1; i < spr.length; i++) {
                            onespr += ', '+spr[i];
                          }
                ...
                if (json.events.length > 0) {
                    ...
                    me.add({
                        ...
                        xtype: 'label',
                        html: '<b style="color:gray; font-size: 12px;">Aufführungen</b>',
                        margin: '10 0 10 0'
                    });
                ...
                if (json.bibl.length > 0) {
                        ...
                        me.add({                       
                        xtype: 'label',
                        html: '<b style="color:gray; font-size: 12px;">Anmerkungen</b>',
                        margin: '10 0 10 0'
                    });
                        var rowText = json.notes;
                        var left_panel_11 = Ext.create('Ext.panel.Panel', {
                        border: false,
                        margin: '0 10 0 10',
                        html: rowText
            ...
            getPersonContentForP = function (personId, personName) {
