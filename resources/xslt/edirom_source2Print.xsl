    ...
    <xsl:variable name="partMeasuresBySurface">
        <xsl:for-each select="//mei:mdiv">
            <div class="partMeasureSurfaceTabel">
                <h3>
                    <xsl:value-of select="@n | @label"><!-- TODO: welches ist der richtige Wert? --></xsl:value-of>
                </h3>
                <xsl:for-each-group select=".//mei:measure" group-by=".//id(if(contains(@facs, '#'))then(substring-after(@facs, '#'))else(./@facs))/parent::mei:surface/@xml:id">
                    <dl>
                        <dt>
                            <xsl:value-of select="current-group()[1]/@n|current-group()[last()]/@n" separator="–"/>
                        </dt>
                        <dd>
...
<!-- TEMPLATEs ============================================================ -->
    ...
    <xsl:template match="/">
        <html>
            <head>
                <title>Print Preview <xsl:value-of select="//mei:source/mei:identifier | //mei:source/mei:titleStmt/mei:title[1]" separator=", "/>
                </title>
    ...
    <xsl:template match="mei:surface">
        <div class="facsimile" id="{@xml:id}">
            <div class="static-before">
                ...
                <span class="header_right">Facsimile-ID: <xsl:value-of select="@xml:id"/>
                </span>
            </div>
            <div class="print-region-body">
                ...
                <div>
                    ...
                    <div class="property">
                        ...
                        <span class="value">
                            <xsl:value-of select="eof:getBarRangeStatement($measuersBySurface/eof:facs[@id = current()/@xml:id]/mei:measure,'–')"/>
                        </span>
                    </div>
                </div>
            </div>
