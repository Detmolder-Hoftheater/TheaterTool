    ...
    createColumn: function () {
        ...
        getSourceContent = function (sourceId, sourceName, workId, refName, selLocation) {
            ...
            var menuItem = historyButton.menu.add({
                text: '<font style="color:gray;">' + refName + ' (' + GUI_NAMES.person_sources_work + ': ' + sourceName + ')' + '</font>', icon: workIcon, dbkey: workId
            });
            ...
            if (! isFoundItem) {
                ...
                var repertoireTab = new TheaterTool.view.tabPanel.HTTab({
                    title: '<font style="color:gray;">' + refName + ' (' + GUI_NAMES.person_sources_work + ': ' + sourceName + ')' + '</font>',
                    icon: workIcon
                });
        ...
        var eColumn = Ext.create('Ext.grid.column.Action', {
            ...
            renderer: function (val, metadata, record) {
                ...
                if (record.data.id !== '') {
                    ...
                    presentationText = '<small style="font-size: 11px; line-height: 1.5em; vertical-align:top;"><a href="javascript:getSourceContent(\'' + record.data.id + '\'' + ', \'' + record.data.name + '\'' + ', \'' + record.data.refId + '\'' + ', \'' + record.data.refName + '\'' + ', \'' + record.data.selLocation + '\');">' + record.data.refName + ' (' + GUI_NAMES.person_sources_work + ': ' + record.data.name + ')' + '</a></small>';
                } else {
                    //this.items[0].icon = '';
                    
                    presentationText = '<small style="font-size: 11px; line-height: 1.5em; vertical-align:top;"> ' + record.data.refName + ' (' + GUI_NAMES.person_sources_work + ': ' + record.data.name + ')' + ' </small>';
                }
                // metadata.style = 'cursor: pointer;';
