...
declare variable $workFolder := request:get-parameter('sourceId', '');
(:declare variable $workFolder := if(contains($selectedWork, 'H020119'))then('aschenbroedel/')else(if(contains($selectedWork, 'H020263'))then('bettelstudent/')else());:)

declare variable $path := concat('xmldb:exist:///apps/',$eoutPath,'/', $workFolder,'/', $fileName, '.xml');
