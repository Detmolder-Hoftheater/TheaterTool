...
declare variable $uri := if($year != '')then(concat('/db/apps/theater-data/einnahmen/', $year, '/'))else();

declare variable $file := collection($uri);

declare variable $uri_1 := if($year != '')then(concat('/db/apps/theater-data/ausgaben/', $year, '/'))else();

declare variable $file_1 := collection($uri_1);

declare variable $path_2 := if($year != '')then(concat('xmldb:exist:///apps/theater-data/spielplaene/', $year, '/'))else('xmldb:exist:///apps/theater-data/spielplaene/');
...
return
    ...
    and $elem/tei:TEI/tei:teiHeader/tei:fileDesc/tei:titleStmt[1]/tei:title = $month)
...
declare function local:dispatch($html_1 as node()*)  as item()* {
 ...
 (: case element(tei:add) return local:addElement($node)
  ...
  case element(tei:measure) return local:measure($node)
...
declare function local:hi($node as element(tei:hi)) as element() {
  ...
  if ($rend = 'bold') then
    ...
    <i>{$node/text()}</i>
        ...
        then(<i>{$node/text()}</i>)
        else(<td width="300px"><i>{$node/text()}</i></td>)
...
if($node/@key != '')then(
<td width="300px">
  <persName  id='{$node/@key}'><a style="color: inherit" href="javascript:getPersonContent('{$node/@key}', '{$node/text()}');">{$node}</a></persName></td>
  )
  else(
  <td width="300px"><persName>{$node}</persName></td>
  )
)else(
if($node/@key != '')then(
  <td width="300px"><persName id='{$node/@key}'><a href="javascript:getPersonContent('{$node/@key}', '{$node/text()}');">{$node}</a></persName></td>
  )
  else(
  <td width="300px"><persName>{$node}</persName></td>
  )
...
if($node/@key != '')then(
  ...
  <td width="300px"><rs id='{$node/@key}'><a href="javascript:getWorkContent('{$node/@key}', '{$node/text()}');">{local:dispatch($node/node())}</a></rs></td>
  )
  else(
  (:<rs>{$node}</rs>:)
  <td width="300px"><rs>{local:dispatch($node/node())}</rs></td>
  )
};
