...
import module namespace api="http://www.hoftheater-detmold.de/xquery/api" at "../../resources/xqm/api.xqm";
...
declare variable $local:swagger-config := json-doc('xmldb:exist://db/apps/TheaterTool/api/v1/swagger.json');
...
declare variable $local:INVALID_PARAMETER := QName("http://www.hoftheater-detmold.de/xquery/api", "ParameterError");
    ...
    return
        element {$root} {
            ...
            return 
                ...
                case function() as item() return <array>{ for $j in map:keys($i) return local:serialize-xml($j, 'root')/node() }</array>
...
return (:(
    ...
    else if($exist:path eq '/' or not($exist:path)) then 
