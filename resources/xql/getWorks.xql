...
(:or @role ="cre"  or @role ="aut"]:)
		...
		let $fileNameCut_1 :=  if($fileNameFiltered_0 = $fileName)
		 ...
		 let $fileNameFiltered_1 :=  if(contains($fileNameCut_1, 'Le ') 
			...
			let $subName := substring($fileNameFiltered, 1,1)
		    let $fileName1 := if(contains($selection, $subName))
			 then($fileName)
			 else():)
		...
		let $source := if(contains($fileID, 'H020119') or contains($fileID, 'H020048')  or contains($fileID, 'H020263'))
		...
		(:let $path_1 := concat('xmldb:exist:///apps/theater-data-exs/expressions/', $expressionFileName, '.xml'):)
		let $file_1 := doc($path_1)
        ...
        let $uri_source := concat('/db/apps/', $sourcetailPath, '/')
		...
		let $sourceName := concat($rismLabel, ' , ' ,$physLoc)
		...
		let $isExtend := if($facsimNames !='')
			...
			let $iconWork := if(contains($fileID, 'H020119') or contains($fileID, 'H020048')  or contains($fileID, 'H020263'))
			then('resources/images/BookBlau-17.png')
			else('resources/images/Books1-17.png')

			let $iconSource := if(contains($fileID, 'H020119') or contains($fileID, 'H020048')  or contains($fileID, 'H020263'))
			then('resources/images/SourceBlue.png')
			else('resources/images/SourceRed.png')

			let $iconIncipits := if(contains($fileID, 'H020119') or contains($fileID, 'H020048')  or contains($fileID, 'H020263'))
			then('resources/images/IncBlue.png')
			else('resources/images/IncRed.png')

			(:let $iconRISM := if(contains($fileID, 'H020119') or contains($fileID, 'H020048')  or contains($fileID, 'H020263'))
